...
namespace DAL
{
    ...
    public class OthersMicrobolgService:BaseService<Models.UserInfo>
    {
        ...
        public IEnumerable<MicroBlogAndUserInfDto> GetOtherBlog(int OtherUserId)
        {
            ...
            var OtherBlogEntity = (from u in DbContexts.UserInfo join mb in DbContexts.MicroBlog on u.Id equals mb.UserID where u.Id == OtherUserId select new MicroBlogAndUserInfDto { UserID = mb.UserID, UserName = u.UserName, UserHeadPortrait = u.UserHeadPortrait, Title = mb.Title, Content_text = mb.Content_text, Content_img = mb.Content_img, Content_sound = mb.Content_sound, Content_video = mb.Content_video, CreateTime = mb.CreateTime, Points_number = mb.Points_number });
